...
//If menuId is found in the path, get menu from database and add to the request and go to next.
...
//Create a new menu with information from request body if title is provided. Else return 400.
...
//Update the menu with requested menuId if title is provided. Else return 400.
...
//Delete menu with requested menuId if it is empty. Else return 400.
menusRouter.delete('/:menuId', (req, res, next) => {
  //Create sql for finding menu items with requested menuId.
  ...
  db.get(menuItemSql, menuItemValues, (error, menuItem) => {
    if (error) {
      next(error);
    } else if (menuItem) { //Menu is not empty
      res.sendStatus(400);
    } else { //Menu is empty. Delete it
